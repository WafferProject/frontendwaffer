...
//   { id: 'delete', label: 'Action', minWidth: 150, align: 'right' },
// ];
...
//               .map((row) => {
//                 return (
//                   <TableRow hover role="checkbox" tabIndex={-1} key={row.id} className={selectedRows.includes(row.id) ? 'selected-row' : ''}>
//                     {columns.map((column) => {
...
//                       <input id={`cbtest-${row.id}`} type="checkbox"   checked={selectedRows.includes(row.id)} onClick={() => handleDeleteRow(row.id)} />
